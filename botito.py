...
async def password(ctx,length: 8):
    # Tell the type checker that User is filled up at this point
    await ctx.send(f"la contraseña es: {gen_pass(length)}")
...
async def coin(ctx):
    ...
    await ctx.send( random.randint(1,2) )
...
@cool.command(name='toby')
async def _bot(ctx):
    """Is the bot cool?"""
    await ctx.send('Yes, the toby is cool.')
